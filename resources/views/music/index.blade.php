...
<div class="bg-black text-white min-h-screen flex">
    ...
    <div id="main-content" class="main-content flex-1 ml-[20%] p-6 bg-black min-h-screen overflow-y-auto">
        ...
        <h2 class="text-2xl font-bold mb-4">Escuchado recientemente</h2>
        <div class="mb-8"> 
            <div class="grid grid-cols-5 gap-4">
                @for ($i = 1; $i <= 5; $i++)
                    <div class="bg-transparent p-4 rounded-lg hover:bg-gray-300 transition duration-300">
                        <img src="mira mi loco esto debe salir de la base de datos" alt="Album Cover" class="w-full h-40 object-cover rounded-md mb-2">
                        <p class="text-sm font-semibold truncate">Canción Escuchada {{ $i }}</p>
                        <p class="text-xs text-gray-400 truncate">Artista {{ $i }}</p>
                    </div>
                @endfor
            </div>
        </div>
        ...
        <h2 class="text-2xl font-bold mb-4">Música Destacada</h2>
        <div class="mb-8"> 
            <div class="grid grid-cols-5 gap-4">
                @for ($i = 1; $i <= 5; $i++)
                    <div class="bg-transparent p-4 rounded-lg hover:bg-gray-300 transition duration-300">
                        <img src="mira mi loco esto debe salir de la base de datos" alt="Album Cover" class="w-full h-40 object-cover rounded-md mb-2">
                        <p class="text-sm font-semibold truncate">Canción Destacada {{ $i }}</p>
                        <p class="text-xs text-gray-400 truncate">Artista {{ $i }}</p>
                    </div>
                @endfor
            </div>
        ...
        <div class="scrollbar-hide -mx-6 mb-8"> 
            <div class="flex space-x-8 pb-4 px-6" style="width: max-content;">
                @for ($i = 1; $i <= 6; $i++)
                    <div class="flex-shrink-0 w-40">
                        <div class="text-center">
                            <img src="mira mi loco esto debe salir de la base de datos" alt="Artista" class="w-48 h-48 rounded-full mx-auto mb-2">
                            <p class="text-sm font-semibold truncate">Artista {{ $i }}</p>
                        </div>
                    </div>
                @endfor
            </div>
